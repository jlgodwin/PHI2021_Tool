...
tract_roomoccupant <- read.csv(
  ...
  colClasses = c("GEOID" = "character", 
                 "value" = "double")) %>% 
  mutate(Race = ifelse(grepl("Two of", Race), "Two or More Races", Race))
...
server <- function(input, output, session) {
  ...
  year_reactive <- reactive({
    if(input$var == "Population"){
      year <- as.numeric(strsplit(input$year_pop, "-")[[1]][1])
    } else if(input$var == "Number of Bedrooms"){
      year <- as.numeric(strsplit(input$year_bed, "-")[[1]][1])
    }  else if(input$var == "Household Size"){
      year <- as.numeric(strsplit(input$year_house_size, "-")[[1]][2])
    } else if(input$var == "Education Level"){
      year <- as.numeric(strsplit(input$year_edu, "-")[[1]][2])
    }else if(input$var == "Rent Burden"){
      year <- as.numeric(strsplit(input$year_rent_b, "-")[[1]][2])
    }else if(input$var == "Median Gross Rent"){
      year <- as.numeric(strsplit(input$year_med_g_rent, "-")[[1]][2])
    }else if(input$var == "Median Income"){
      year <- as.numeric(strsplit(input$year_med_income, "-")[[1]][2])
    }else if(input$var == "Number of Occupants per Room"){
  ...
  all_selected <- reactive({
    input$measure_type_cat == "Prevalence" &&
  ...
  geo_year_df_reactive <- reactive({
    ...
    geo_year_df <- geo_df %>%
      ...
      group_by(GEOID) %>%
      mutate(
        prev_total = sum(value, na.rm = TRUE)
      ) %>%
      ungroup()
  ...
  measure_df_reactive <- reactive({
    ...
    if(!straight_pipe_hh()) {
      ...
      if (measure == "Value") {
        ...
        measure_df <- selected_df %>%
          mutate(
            #           moe = round(moe / qnorm(.95) *10^4) / 10^2,
            # value = round(value / sum(value) * 10^4) / 10^2,
            value = round(value / sum(value) * 10^4),
  ...
  popup_text_reactive <- reactive({
    ...
    #                      "Tract No.: ", "HRA: ")
    geo_prefix <- if(geo_logical== "Census Tract"){
      "Tract ID: "
    }else{
      "HRA: "
    }
    if (measure == "Count"){
      ...
      measure_prefix <- "Distribution Precentage: "
    }else if (measure  == "Value"){
      if (straight_pipe_hh()) {
  ...
  legend_values_reactive <- reactive({
    ...
    if(input$measure_type_cat == "Distribution"){
      quantile(sp %>% 
                 as_tibble() %>% 
                 select(value) %>% 
                 unlist(), type = 5, 
  ...
  output$map <- renderLeaflet({
    ...
    kc_cr_station_2040 <- read_sf("./data/kc_cr_station_2040.shp")
    # 
    # # load light rail station GIS data
    kc_lr_station_2040 <- read_sf("./data/kc_lr_station_2040.shp")
    # 
    # # load public clinics GIS data
    kc_public_clinics <- read_sf("./data/kc_public_clinics.shp")
    # 
    # # load Women, Infant and Children Services GIS data
    kc_wic <- read_sf("./data/kc_wic.shp")
    # 
